import Dating from "../public/assets/card-image/image (1).webp";
import Networking from "../public/assets/card-image/image (2).webp";
import Events from "../public/assets/card-image/image (3).webp";
import Matches from "../public/assets/card-image/image (4).webp";

export const HOME_CAROUSEL_DATA = [
  {
    src: "/assets/carousel_images/01.webp",
    heading: "Find, meet similar people and develop meaningful connections",
    subHeading:
      "World’s first all-in-one AI driven app for dating, networking and events",
  },
  {
    src: "/assets/carousel_images/02.webp",
    heading: "Date  Profile",
    subHeading:
      "Hot singles are waiting for you! Cut through the noise & build happy relationships",
  },
  {
    src: "/assets/carousel_images/03.webp",
    heading: "Professional Profile",
    subHeading:
      "Recruiters, Freelancers,Blogs and everything to increase your network.",
  },
];

export const WHY_MINGLE_WISE = [
  {
    image: Dating,
    type: "Dating",
    info: "Find exciting people, meet in real or video dates & try your luck with speed dating!",
  },
  {
    image: Networking,
    type: "Professional Networking",
    info: "Find like-minded professionals, grow your network & reach your goals!",
  },
  {
    image: Events,
    type: "Events",
    info: "Participate in virtual or hybrid events using event codes. Host & livestream your events to your networks!",
  },
  {
    image: Matches,
    type: "Verified & Compatible matches",
    info: "Connect with verified & compatible people that will increase probability of forming truly meaningful relationships!",
  },
];

export const DATING_PROFILE = [
  {
    logo: "/assets/dating-profile/schedule 1.svg",
    title: "Events",
    info: "Create your own events or attend events Mingle Video or Mingle Voice.",
  },
  {
    logo: "/assets/dating-profile/explore.svg",
    title: "Explore",
    info: "Find amazing people all over the app which are hot, active and new.",
  },
  {
    logo: "/assets/dating-profile/mingle-instant.svg",
    title: "Mingle Instant",
    info: "Participate in an exciting on the spot video and chat calls with other users. Connect just by selecting the gender and location.",
  },
  {
    logo: "/assets/dating-profile/feed.svg",
    title: "Feed",
    info: "Swipe up and down for the profiles you like or dislike and talk to them.",
  },
  // {
  //   logo: "/assets/dating-profile/Games.svg",
  //   title: "Games",
  //   info: "Enjoy fun games in the game room with your date.",
  // },
  // {
  //   logo: "/assets/dating-profile/feed.svg",
  //   title: "Experiences",
  //   info: "Experiences feature allows users to participate in to meet new people and potentially find romantic connections. These experiences are virtual ,IRL Experiences and user can create there own.",
  // },
  {
    logo: "/assets/dating-profile/chats.svg",
    title: "Chats",
    info: "Send mystery messages to your match and impress your date.",
  },
  {
    logo: "/assets/dating-profile/speed-dating.svg",
    title: "Speed Dating",
    info: "A series of short conversations lasting for 3 minutes each with 5 different people.",
  },
  {
    logo: "/assets/dating-profile/m-coins.svg",
    title: "M Coins",
    info: "Buy some M coins to access popular features like Mingle Match, Mingle Meet & Mingle Speed",
  },
];
export const PROFESSIONAL_NETWORKING = [
  {
    logo: "/assets/professional-networking/schedule 1.svg",
    title: "Events",
    info: "Create your own events or attend events Mingle Video or Mingle Voice.",
  },
  {
    logo: "/assets/professional-networking/explore.svg",
    title: "Explore",
    info: "Find amazing people all over the app which are hot active and new.",
  },
  // {
  //   logo: "/assets/professional-networking/mingle-instant.svg",
  //   title: "Mingle Instant",
  //   info: "Participate in an exciting on the spot video and chat calls with other users. Connect just by selecting the gender and location.",
  // },
  // {
  //   logo: "/assets/professional-networking/Mask group.svg",
  //   title: "Feed",
  //   info: "Swipe up and down for the profiles you like or dislike and talk to them.",
  // },
  // {
  //   logo: "/assets/professional-networking/globe.svg",
  //   title: "Speed Networking",
  //   info: "A series of short conversations lasting for 3 minutes each with 5 different people.",
  // },
  {
    logo: "/assets/professional-networking/paper-plane 1.svg",
    title: "Chats",
    info: "Send mystery messages and connect with like minded people.",
  },
  {
    logo: "/assets/professional-networking/briefcase 1.svg",
    title: "Jobs",
    info: "Find Jobs according to your field and internships as well.",
  },
  {
    logo: "/assets/professional-networking/freelance-work.svg",
    title: "Freelance Work",
    info: "Create your gigs find freelance work and get started.",
  },
];
export const IN_APP_FEATURES = [
  {
    logo: "/assets/in-app-features/ICON.svg",
    title: "Compatibility",
    info: "Get a detailed summary of how you match up with the other user.",
  },
  {
    logo: "/assets/in-app-features/privacy.svg",
    title: "Profile Privacy",
    info: "Choose what you want to share with more privacy control.",
  },
  {
    logo: "/assets/in-app-features/mic.svg",
    title: "Mingle Instant",
    info: "Participate in an exciting on the spot video and chat calls with other users. Connect just by selecting the gender and location.",
  },
  {
    logo: "/assets/in-app-features/badge.svg",
    title: "Elite Badge",
    info: "Here, by using this feature you can get discounts on in-app purchases like meet, speed, gift and lime light.",
  },
  {
    logo: "/assets/in-app-features/limelight.svg",
    title: "Limelight",
    info: "By using this feature, top profiles will be visible to you. By using this, you explore top profiles.",
  },
  {
    logo: "/assets/in-app-features/stories.svg",
    title: "Stories",
    info: "Post your story in the form photos or videos. Also, to make your stories more fun and interactive, you can use features like stickers, hashtags, gifs, music, etc.",
  },
  {
    logo: "/assets/in-app-features/location.svg",
    title: "Mingle Mixer",
    info: "See mutual connections with your matches.",
  },
  {
    logo: "/assets/in-app-features/coupons.svg",
    title: "Coupons",
    info: "See mutual connections with your matches.",
  },
];

export const REVIEW_DATA = [
  {
    image: "/assets/review-iamge/Picture1.webp",
    title: "Found My Kind Of People",
    name: "Mannat Jaswal",
    info: "This is a good platform for attending events and also has a good interface. I enjoyed using this app.",
  },
  {
    image: "/assets/review-iamge/Picture2.webp",
    title: "A Great Way To Explore New Hobbies",
    name: "Harman",
    info: "Helps you meet new people who are like minded. Finds great matches. Overall a good experience.",
  },
  {
    image: "/assets/review-iamge/Picture3.webp",
    title: "Perfect Platform For Community Events",
    name: "Mamta Dhanda",
    info: "This is the best app for dating and networking. Very satisfying.",
  },
  {
    image: "/assets/review-iamge/Picture4.webp",
    title: "Found My Kind Of People",
    name: "Anchal Verma",
    info: "It is really good dating app. Through this app we connected really good people and we know ourselves from others.",
  },
  {
    image: "/assets/review-iamge/Picture5.webp",
    title: "A Great Way To Explore New Hobbies",
    name: "Disha Jain",
    info: "Great app to organize and manage events.",
  },
  {
    image: "/assets/review-iamge/Picture6.webp",
    title: "Perfect Platform For Community Events",
    name: "Neeru Rastogi",
    info: "Great platform to organize and attend events . An amazing app to find social connections .",
  },
  {
    image: "/assets/review-iamge/Picture7.webp",
    title: "Found My Kind Of People",
    name: "Ch Shobha Pal",
    info: "Great platform to organize and attend a event.",
  },
  {
    image: "/assets/review-iamge/Picture8.webp",
    title: "A Great Way To Explore New Hobbies",
    name: "DIVYANSH SINGH",
    info: "Very strong app with a very wise motive",
  },
  {
    image: "/assets/review-iamge/Picture9.webp",
    title: "Perfect Platform For Community Events",
    name: "Kunal Jain 236",
    info: "Overall experience and user friendly interface is pretty good Really enjoyed this app!!",
  },
];
